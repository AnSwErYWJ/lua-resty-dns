...
function _M.new(class, opts)
    ...
    for i = 1, n do
        ...
        local ok, err = sock:setpeername(host, port) --if unconnected, recorder peer ip and port ; else, clean connect.
        if not ok then
...
local function parse_response(buf, id, opts)
    ...
    if ans_id ~= id then
        ...
        print(DEBUG, "id mismatch in the DNS reply: ", ans_id, " ~= ", id)
...
local function _tcp_query(self, query, id, opts)
    ...
    print(DEBUG, "query the TCP server due to reply truncation")
...
function _M.query(self, qname, opts)
    ...
    -- print("query: ", json.encode(concat(query, "")))
